...
class CategorytileState extends State<Categorytile> {
  ...
  Future<void> initializeFavorites() async {
    try {
      ...
      setState(() {
        favoriteList = List<bool>.filled(
          widget.snapshot.data!.length,
          false,
          growable: true,
        );
        for (int i = 0; i < widget.snapshot.data!.length; i++) {
          if (wishlistProductIds.contains(widget.snapshot.data![i].id)) {
            favoriteList[i] = true;
          }
        }
      });
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        bool isFavorite = favoriteList.length > index ? favoriteList[index] : false;
