...
import 'package:relt/controller/favourite_page.dart';
...
class _WishlistScreenState extends State<WishlistScreen> {
  ...
  void initState() {
    super.initState(); 
    _wishlistController.initializeWishlistData();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("WISHLIST",textAlign: TextAlign.center ,),backgroundColor: Colors.black,
    
      ),
      body: RefreshIndicator(
        onRefresh: _wishlistController.initializeWishlistData,
        child: StreamBuilder<WishlistModel?>(
          stream: _wishlistController.wishlistModelStream,
          builder: (context, snapshot) {
            ...
            WishlistModel wishlistModel = snapshot.data!;

            if (wishlistModel.wishlistProducts.isEmpty) {
              return const Center(
                child: Text('No products in wishlist'),
              );
            }

            return CategoryTile(wishlistModel);
          },
        ),
      ),
    ); 
...
class CategoryTile extends StatefulWidget {
  ...
  const CategoryTile(this.wishlistModel, {Key? key}) : super(key: key);
...
class _CategoryTileState extends State<CategoryTile> {
  ...
  Future<void> initializeFavorites() async {
    try {
      List<String> wishlistProductIds =
          widget.wishlistModel.wishlistProducts.map((documentSnapshot) => documentSnapshot.id).toList();

      setState(() {
        favoriteList = List<bool>.generate(
          widget.wishlistModel.wishlistProducts.length,
          (index) => wishlistProductIds.contains(widget.wishlistModel.wishlistProducts[index].id),
        );
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      itemCount: widget.wishlistModel.wishlistProducts.length,
      ...
      itemBuilder: (context, index) {
        DocumentSnapshot<Object?> productSnapshot = widget.wishlistModel.wishlistProducts[index];
  ...
  void _deleteProductFromWishlist(
    ...
    wishlistManager.toggleWishlistStatus(productSnapshot, false).then((_) {
      ...
      setState(() {
        widget.wishlistModel.wishlistProducts.remove(productSnapshot);
